...
各ページに主役をひとり。主役 (時計・タイマー・現在気温) は箱に入れず
背景の上に直接置き、ガラスカードは境界が必要な脇役だけに使う
(`filled: false` が主役、`true` が脇役のパネル)。主役の数字は
領域の高さ × `T.heroFrac` で自動サイズになるので、どのページでも格が揃う。

同じ情報を複数ページに置かない。ホームは要約で、タップを詳細への
入口にする (`tapPage`)。ホームの天気カードと気温スパークラインを
タップすると天気詳細ページへ飛ぶ。

週間予報は Apple 天気の週間欄 (曜日|アイコン|降水%|最低—帯—最高)、
時間別は iOS の時間別ウィジェット (時刻|アイコン|気温) の型に合わせる。
天気はテキストではなくアイコン (Canvas の単色シルエット) で示す。
...
| `pomodoro` | — |
| `weather.now` | `entity`。アイコン + 気温が主役、天気の言葉と湿度は1行に従属 |
| `weather.forecast` | `entity`, `forecastType` (`daily`/`hourly`), `count` (5, 1-8)。daily は Apple 天気式の行 (曜日\|アイコン\|降水%\|最低—帯—最高)、hourly は時刻\|アイコン\|気温の短冊 |
...
`showLabel`、`label`、`tapPage` (タップで指定した名前のページへ移動)。
